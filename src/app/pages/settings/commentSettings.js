...
const OldPostsComments = ({ setError, notify }) => {
	...
	const disableCommentsNoticeTitle = () => {
		return disableCommentsOldPosts
			? __('Disabled old post comments', 'wp-plugin-crazy-domains')
			: __('Enabled old post comments', 'wp-plugin-crazy-domains');
	};
	...
	const toggleDisableCommentsOldPosts = () => {
		crazydomainsSettingsApiFetch({ disableCommentsOldPosts: !disableCommentsOldPosts }, setError, (response) => {
...
const CloseCommentsDays = ({ setError, notify }) => {
	...
	const closeCommentsDaysNoticeTitle = () => {
		return (
			__('Comments setting saved ', 'wp-plugin-crazy-domains')
		);
	};
	...
	const closeCommentsDaysNoticeText = () => {
		...
		return (
			__('Comments on posts are disabled after ', 'wp-plugin-crazy-domains') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-crazy-domains')
		);
	};
	...
	const handleCloseCommentsDaysChange = (value) => {
		crazydomainsSettingsApiFetch({ closeCommentsDays: value }, setError, (response) => {
...
const CommentsPerPage = ({ setError, notify }) => {
	...
	const commentsPerPageNoticeTitle = () => {
		return __('Comments setting saved.', 'wp-plugin-crazy-domains');
	};
	...
	const handleCommentsPerPageChange = (value) => {
		crazydomainsSettingsApiFetch({ commentsPerPage: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Display ', 'wp-plugin-crazy-domains') + commentsPerPage +
