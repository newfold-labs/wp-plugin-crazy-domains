			...
			if ( function_exists( 'is_plugin_active' ) && is_plugin_active( $incompatible_file ) && ! in_array( $this->slug, $conflict_plugins, true ) ) {
				...
				$error->add(
					...
					sprintf(
						...
						__( '"%1$s" has self-deactivated. It is incompatible with "%2$s".', 'wp-plugin-crazy-domains' ),
						$this->name,
						$incompatible_name
					)
				);
			...
			if ( function_exists( 'is_plugin_active' ) && is_plugin_active( $legacy_file ) && ! in_array( $legacy_file, $conflict_plugins, true ) ) {
				...
				$error->add(
					...
					sprintf(
						...
						__( '"%1$s" has been deactivated. It is incompatible with "%2$s".', 'wp-plugin-crazy-domains' ),
						$legacy_name,
						$this->name
					)
				);
